...
public class WifiapBroadcast extends BroadcastReceiver {
	...
	private final static String tag = "WifiapBroadcast";

	public void onReceive(Context paramContext, Intent paramIntent) {

		if (paramIntent.getAction().equals( // 搜索到wifi热点
				WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
			Log.e(tag, "SCAN_RESULTS_WIFI-------------");
			...
			Log.e(tag, "WIFI opened or closed-------------");
			...
			ConnectivityManager connectMgr = (ConnectivityManager) paramContext
					.getSystemService(paramContext.CONNECTIVITY_SERVICE);
			NetworkInfo wifiNetInfo = connectMgr
					.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
			if (!wifiNetInfo.isConnected()) {
				for (int i = 0; i < ehList.size(); i++)
					((EventHandler) ehList.get(i)).wifiDisconnect();
			}else if(wifiNetInfo.isConnected()){
				Log.e(tag, "WIFI is connect------------");
			}
		}
